...
path_federated = Path("../federatedCallback/")
...
path_to_config = path_federated/f"tmp_federated_communication/{prefix.replace(clinic, '')}config.conf"
...
def training_procedure(sample_rate_majority_class, epochs_frozen, epochs_unfrozen, learning_rate, sync_factor, random_erasing_max_count, random_erasing_probability, experiment):
    ...
    for i in clinic_list:
        slides_per_state_dict[f"{path_federated}/tmp_state_dicts/{model_name}_{i}_{subname}_state_dict.pkl"] = slides_per_clinic_dict[i]
        # print(f"{path_federated}/tmp_state_dicts/{model_name}_{i}_{subname}_state_dict.pkl has{slides_per_clinic_dict[i]} slides!")
    ...
    my_cbs = [SamplerCallback(**dataloader_args),
              ...
              FederatedCallback(sync_factor=sync_factor, state_dicts_dir=f"{path_federated}/tmp_state_dicts/", communication_dir=f"{path_federated}/tmp_federated_communication/",
...
def check_for_loaded_config():
    ...
    for c in clinic_list:
        if c != leading_clinic:
            config_signal_list.append(path_federated/f"tmp_federated_communication/{prefix.replace(clinic, c)}loaded")
...
if is_sync_node:
    do_del_paths = glob.glob(f"{str(path_federated)}/tmp_federated_communication/{model_name}*_{subname}*")
